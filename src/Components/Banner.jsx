

const Banner = () => {
  return (
    <div className="md:h-[650px] w-full bg-[linear-gradient(to_top,rgba(00,00,00,0.2),rgba(00,00,00,0.5)),url('https://i.ibb.co/2KM2ZyM/rsz-1pexels-vlada-karpovich-4050302.jpg')] text-white bg-cover rounded-md flex justify-center items-center p-5 bg-black">
      <div className=" p-10 w-4/6 space-y-12 text-center m-auto">
        <h1 className="md:text-6xl text-3xl leading-loose  font-bold">
          Online Course Can You Purchase or Sell
        </h1>
        <p className="text-xl">
          Lorem ipsum dolor, sit amet consectetur adipisicing elit. Quam dolores
          eum unde ullam qui praesentium quia vero, odio quo ad!
        </p>
        
      </div>
    </div>
  );
};



export default Banner;
