...
const MySessionDetails = () => {
  ...
  const { data: sessionDetails = {}, isLoading: sessionLoading } = useQuery({
    ...
    enabled: !!id, // Ensure id is available before running query
  });
  ...
  const handleSubmitReview = (event) => {
    ...
    axiosSecure
      .put(`save-ratings/${sessionId}`, ratingsData)
      .then((res) => {
        ...
        // Optionally, you can also fetch updated session details after the review submission
        // This ensures that the UI reflects the latest data
        // queryClient.invalidateQueries(["sessionDetails", id]);
      })
      .catch((error) => {
